...
/**
 * @param {{ includeDrafts?: boolean, highlightsOnly?: boolean, limit?: number }} [options]
 */
export function usePortfolioProjects(options = {}) {
  const { includeDrafts = false, highlightsOnly = false, limit = null } = options;
  ...
  const load = useCallback(async () => {
    ...
    setLoading(false);
  }, [includeDrafts, highlightsOnly, limit]);
